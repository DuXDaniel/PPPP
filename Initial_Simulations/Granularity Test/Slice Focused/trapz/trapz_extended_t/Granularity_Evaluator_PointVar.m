...
points_to_add = 100:100:2000;

voxel_gran_length = 3:2:9;
slice_gran_length = 3:2:43;
%slice_string = [strsplit(num2str(slice_gran_length(1:100).^2),' '), '10000'];
% f1 = figure;
f2 = figure;
% f3 = figure;
...
sizeofgrid = ceil(sqrt(36));

for k = 1:length(points_to_add)
    for i = 2%1:length(voxel_gran_length)
        for j = 1:length(slice_gran_length)
            ...
            assign_var_name = ['Quasiclassical_Func_Double_Laser_V01_SM_VarVox_1_1_2_3_',num2str(i),'_',num2str(j),'_',num2str(k)];
            try
                load(assign_var_name);
                phase_shifts = phase - min(min(phase));
                %length(voxel_gran_length),length(slice_gran_length),(i-1)*length(slice_gran_length) + j
                subplot(3,3,j);
            catch
                break;
            end
            ...
            %         j
            %
            %         assign_var_name = ['Feynman_Func_Double_Laser_V01_SM_VarVox_1_1_1_3_1_',num2str(j)];
            %         try
            %             load(assign_var_name);
            %             phase_shifts = phase - min(min(phase));
            %             subplot(sizeofgrid,sizeofgrid,(i-1)*length(slice_gran_length) + j);
