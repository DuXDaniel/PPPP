...
for i = 1:length(laser_res_range)
    for j = 1:length(e_res_range)
        ...
        for l = 10%:length(laser_pulse_range)
            for m = 1%:length(laser_width_range)
                ...
                [phase,t_range] = Final_Phase_Data_Converter(laser_width_range(m), laser_res_range(i), e_res_range(j), laser_pulse_range(l), slices);
                ...
                if ~isnan(slices)
                    ...
                    for n = 1:size(slices,3)
                        subplot(11,11,n);
                        imagesc(flipud(rot90(slices(:,:,n),1)));
                        xticks = [];
                        yticks = [];
                        xtick_labels = [];
                        ytick_labels = [];
                        ...
                        %                         title([set_name title_name], 'Interpreter', 'none');
                        set(gcf,'Position',[0 0 1600 1600]);
                        title(num2str(max(max(slices(:,:,n)))));
                        pbaspect([1 1 1]);
                    end
...
%                         %                         title([set_name title_name], 'Interpreter', 'none');
%                         set(gcf,'Position',[0 0 1600 1600]);
%                         title([num2str(t_range(n)),'ps']);
%                         pbaspect([1 1 1]);
%                     end
%                     print([set_name title_name '_times'],'-dpng','-r600')
%                     close(g);
                end
            end
        end
