function [final_phase_data] = Simul_Script_Test_V4_Func(beam_waist, laser_res, e_res, pulse_energy)
...
laser_sum = @(t) integral2(@(x,y) 2.*pi.*x.*laser(x,y,t), 0, 3*(omeg_las_sq(c*t)+sig_las*c), -3*(sig_las*c+z0)-c*t, 3*(sig_las*c+z0)+c*t);
...
for cur_slice = 1:voxel_granularity
    ...
    for m = 1:voxel_granularity
        parfor n = 1:voxel_granularity
            ...
            full_func = @(t) hbar.*alpha.*n_pulse./((0 == laser_sum(t)).*1e308 + laser_sum(t)).*laser(rho_func(t),z_func(t),t).*lambda./sqrt(mass_e^2.*(1+vel.^2/c.^2)); %
            
            %%
            
            voxel_grid_phase_data(m,n,cur_slice) = integral(@(t) full_func(t), -6*sig_ebeam, 6*sig_ebeam,'ArrayValued',true);
...
for m = 1:voxel_granularity
    for n = 1:voxel_granularity
        for p = 1:voxel_granularity
            final_phase_data(m,n) = final_phase_data(m,n) + voxel_grid_phase_data(m,n,:).*voxel_y_weights(p);
        end
    end
end
