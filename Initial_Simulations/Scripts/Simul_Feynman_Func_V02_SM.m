function [final_phase_data, voxel_grid_phase_data] = Simul_Feynman_Func_V02_SM(beam_waist, laser_res, e_res, pulse_energy)
...
%% Unpacking slopes and initial y values
...
for cur_voxel = 1:num_voxels
    ...
    %%
    integral_waypoints = [-abs(init_y_vals(cur_voxel))/vel,0,abs(init_y_vals(cur_voxel))/vel];
    ...
    waitbar(cur_voxel/num_voxels,h2,'Calculating phase shift for each slice');
end
