function [voxel_grid_cumulative_phase_data] = Simul_Feynman_Func_Sized_Double_Laser_UniformE_V01_SM(beam_waist, laser_res, pulse_energy)
...
laser_sum = @(t) integral2(@(x,y) 2.*pi.*x.*laser(x,y,t), 0, gauss_limit*sqrt(omeg_las_sq(c*t)), t*(-gauss_limit*sig_las + c), t*(gauss_limit*sig_las + c));
...
for zin = 1:length(z_drift)
    ...
    for xin = 1:length(x_drift)
        ...
        parfor cur_voxel = 1:num_voxels
            ...
            x_func = @(t) y_func(t).*x_slopes(cur_voxel) + x_drift(xin);
            z_func = @(t) y_func(t).*z_slopes(cur_voxel) + z_drift(zin);
            ...
            full_func = @(t) hbar.*alpha.*n_pulse.*norm_factor(t).*(laser(rho_func(t),z_func(t),t)+laser_x_dir(rho_func_2(t),x_func(t),t)).*lambda./sqrt(mass_e^2.*(1+vel.^2/c.^2)); %
