function [final_phase_data] = Simul_Script_Test_V5_Func(beam_waist, laser_res, e_res, pulse_energy)
...
sig_ebeam = e_res; % time resolution of ebeam, seconds
...
%% Form taylor expansion (or maybe a Gaussian expansion?) for time-dependent normalization factor
t_range = -50*sig_ebeam:min(sig_ebeam,sig_las)/voxel_granularity:50*sig_ebeam;
...
assignin('base','voxel_grid_slope_z_data',voxel_grid_slope_z_data);
...
for cur_slice = 1:voxel_granularity
    ...
    for m = 1:voxel_granularity
        parfor n = 1:voxel_granularity
            ...
            voxel_grid_phase_data(m,n,cur_slice) = integral(@(t) full_func(t), -gauss_limit*sig_ebeam, gauss_limit*sig_ebeam);
