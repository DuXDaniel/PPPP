...
assign_var_name = 'Simul_Feynman_Func_Sized_sinlas_nslas_nse_100um_1nJ_SM';
load(assign_var_name);

phase_Feynman = phase;
Feynman_phase_dif = phase_Feynman - min(min(phase_Feynman));

assign_var_name = 'Quasiclassical_Func_Sized_sinlas_nslas_nse_100um_1_SM';
load(assign_var_name);

phase_Quasi = phase;
Quasi_phase_dif = phase_Quasi - min(min(phase_Quasi));
Quasi_phase_norm = Quasi_phase_dif./max(max(Quasi_phase_dif));
...
if ~isnan(phase)
    ...
    phase_mult = Feynman_phase_dif.*Quasi_phase_norm;
    imagesc(phase_mult);
    xticks = [];
    yticks = [];
    xtick_labels = [];
    ...
    colormap(cm_viridis);
    cbh = colorbar;
    cbh.Ticks = [min(min(phase_mult)) max(max(phase_mult))];
    cbh.TickLabels = [round(min(min(phase_mult))) max(max(phase_mult))];
    cbh.FontSize = 16;
    title(assign_var_name, 'Interpreter', 'none');
    set(gcf,'Position',[0 0 1600 1600]);
    pbaspect([1 1 1]);
    print('sinlas_pulsed_convolution','-dpng','-r600')
    close(f);
...
%             for m = 1:length(laser_width_range)
%
%                 assign_var_name = ['Quasiclassical_Func_Double_Laser_V01','_',num2str(i),'_',num2str(j),'_',num2str(l),'_',num2str(m)];
