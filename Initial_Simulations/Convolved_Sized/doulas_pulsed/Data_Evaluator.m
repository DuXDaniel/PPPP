...
assign_var_name = 'Simul_Feynman_Func_Sized_doulas_nslas_nse_100um_1nJ_SM';
load(assign_var_name);

phase_Feynman = phase;
Feynman_phase_dif = phase_Feynman - min(min(phase_Feynman));

assign_var_name = 'Quasiclassical_Func_Sized_doulas_nslas_nse_100um_1_SM';
load(assign_var_name);
...
if ~isnan(phase)
    ...
    pbaspect([1 1 1]);
    print('doulas_pulsed_convolution','-dpng','-r600')
    close(f);
