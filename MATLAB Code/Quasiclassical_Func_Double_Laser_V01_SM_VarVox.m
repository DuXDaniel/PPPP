function [final_phase_data, voxel_grid_phase_data] = Quasiclassical_Func_Double_Laser_V01_SM(beam_waist, laser_res, e_res, prefactor, voxgran, slicegran, points_to_add)
...
sum(voxel_y_weights)
assignin('base','e_beam_yt_norm',e_beam_yt_norm);
assignin('base','y_bounds',y_bounds);
...
min_t_vals_sep = points_to_add;%round(min_t_vals/slice_granularity) + slice_granularity;
...
voxel_grid_phase_data_unpacked = test_parfor_mem_load(num_voxels,init_y_vals,x_slopes,z_slopes,t_range_extended,norm_factor_array,laser,norm_func,beta,c,lambda,theta,integral_bound,t_bounds,vel);
