...
for cur_slice = 1:voxel_granularity
    ...
    for m = 1:voxel_granularity
        ...
        for n = 1:voxel_granularity
            ...
            laser_norm = @(t) n_pulse./integral2(@(rho_xy,z) laser(rho_xy,z,t), 0, Inf, -Inf, Inf); % problem child --> throwing error with matrix dimensions
            ...
            omeg_las_sq = @(t) w0.^2.*(1+z_func(t).^2./z0.^2);
            
            laser_t = @(t) 1./pi.^2./omeg_las_sq(t)./sig_las.^2./c.^2.*exp(-1.*(2.*rho_func(t).^2)./(omeg_las_sq(t))).*exp(-1.*(2.*(z_func(t)-c.*t).^2)./(sig_las.^2.*c.^2));
        
            laser_func = @(t) laser_norm(t).*laser_t(t);
            full_func = @(t) hbar.*alpha.*laser_func(t).*lambda./sqrt(mass_e.^2.*(1+vel.^2./c.^2));
