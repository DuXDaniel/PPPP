    ...
    for j = 1:length(e_res_range)
        for l = 1:length(laser_pulse_range)
            for m = 1:length(laser_width_range)
                [phase] = Quasiclassical_Func_V01_SM(laser_width_range(m), laser_res_range(i), e_res_range(j), laser_pulse_range(l));
                
                assign_var_name = ['Quasiclassical_Func_V01_SM',num2str(i),'_',num2str(j),'_',num2str(l),'_',num2str(m)];
