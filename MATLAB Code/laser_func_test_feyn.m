...
for cur_slice = 1:slice_granularity
    ...
    for m = round(voxel_granularity/2)-5
        for n = round(voxel_granularity/2)-5
            ...
            full_laser = @(t) laser(rho_func(t),z_func(t),t);
            
            subplot(2,2,1);
            plot(t_bounds,-(2*rho_func(t_bounds).^2)./(omeg_las_sq(z_func(t_bounds))./temporal_gauss(z_func(t_bounds),t_bounds)));
            hold on;
            subplot(2,2,2);
            plot(t_bounds,temporal_gauss(z_func(t_bounds),t_bounds));
            hold on;
            subplot(2,2,3);
            plot(t_bounds,spatial_gauss(rho_func(t_bounds),z_func(t_bounds),t_bounds));
            hold on;
            subplot(2,2,4);
