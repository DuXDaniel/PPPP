function [voxel_grid_cumulative_phase_data] = Quasiclassical_Func_Sized_Double_Laser_UniformE_V01_SM(beam_waist, laser_res, e_res, prefactor)
...
for zin = 1:length(z_drift)
    ...
    for xin = 1:length(x_drift)
        ...
        parfor cur_voxel = 1:num_voxels
            ...
            full_func = @(t) (norm_factor(t).*laser(rho_func(t),z_func(t),t)).^2.*(1-beta.^2.*cos(2.*pi.*(z_func(t)-c.*t)/lambda).^2.*cos(theta).^2) + (norm_factor(t).*laser_x_dir(rho_func_2(t),x_func(t),t)).^2.*(1-beta.^2.*cos(2.*pi.*(x_func(t)-c.*t)/lambda).^2.*cos(theta).^2); %
