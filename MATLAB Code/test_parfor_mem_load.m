function [voxel_grid_phase_data_unpacked] = test_parfor_mem_load(num_voxels,init_y_vals,x_slopes,z_slopes,t_range,norm_factor_array,laser,norm_func,beta,c,lambda,theta,integral_bound,t_bounds,vel)
...
parfor cur_voxel = 1:num_voxels
    ...
    full_vals = (norm_func_vals.*laser(rho_vals,z_vals,t_range_gpu)).^2.*(1-beta.^2.*cos(2.*pi.*(z_vals-c.*t_range_gpu)/lambda).^2.*cos(theta).^2) + (norm_func_vals.*laser(rho_vals_2,x_vals,t_range_gpu)).^2.*(1-beta.^2.*cos(2.*pi.*(x_vals-c.*t_range_gpu)/lambda).^2.*cos(theta).^2);
    voxel_grid_phase_data_unpacked(cur_voxel) = trapz(t_range_gpu,full_vals);
end

% voxel_grid_phase_data_unpacked = zeros(num_voxels,1);
% norm_func_vals = norm_func(t_range);
% % init_y_vals_gpu = gpuArray(init_y_vals);
% % x_slopes_gpu = gpuArray(x_slopes);
% % z_slopes_gpu = gpuArray(z_slopes);
% % t_range_gpu = gpuArray(t_range);
...
%     rho_vals = sqrt(x_vals.^2+y_vals.^2);
%     rho_vals_2 = sqrt(z_vals.^2+y_vals.^2);
%     full_vals = (norm_func_vals.*laser(rho_vals,z_vals,t_range)).^2.*(1-beta.^2.*cos(2.*pi.*(z_vals-c.*t_range)/lambda).^2.*cos(theta).^2) + (norm_func_vals.*laser(rho_vals_2,x_vals,t_range)).^2.*(1-beta.^2.*cos(2.*pi.*(x_vals-c.*t_range)/lambda).^2.*cos(theta).^2);
%     voxel_grid_phase_data_unpacked(cur_voxel) = trapz(t_range,full_vals);
% end
% 
