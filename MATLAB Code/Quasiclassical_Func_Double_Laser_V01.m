function [final_phase_data] = Quasiclassical_Func_V01(beam_waist, laser_res, e_res, prefactor)
...
for cur_slice = 1:slice_granularity
    ...
    for m = 1:voxel_granularity
        for n = 1:voxel_granularity
            ...
            %%
            full_func = @(t) (laser(rho_func(t),z_func(t),t)).^2.*(1-beta.^2.*cos(2.*pi.*(z_func(t)-c.*t)/lambda).^2.*cos(theta).^2) + (laser(rho_func_2(t),x_func(t),t)).^2.*(1-beta.^2.*cos(2.*pi.*(x_func(t)-c.*t)/lambda).^2.*cos(theta).^2); %
