...
laser_pulse_range = logspace(1,3,3);
...
%slice_gran_length = slice_gran_length(1:9);

for i = 3%1:length(voxel_gran_length)
    for j = 3%1:length(slice_gran_length)
%         [phase, slices] = Simul_Feynman_Func_V01_SM_VarVox(las_width_int, las_res_int, e_res_int, las_pulse_int, voxel_gran_length(i)^2, slice_gran_length(j)^2);
%         
%         assign_var_name = ['Feynman_Func_V01_SM_VarVox_1_1_1_3_',num2str(i),'_',num2str(j)];
%         save(assign_var_name,'phase','slices');
        
        [phase, slices] = Simul_Feynman_Dual_Laser_Func_V01_SM_VarVox(las_width_int, las_res_int, e_res_int, las_pulse_int, voxel_gran_length(i)^2, slice_gran_length(j)^2);
