...
for i = 1:length(laser_res_range)
    for l = 1:length(laser_pulse_range)
        for m = 1:length(laser_width_range)
            [phase] = Simul_Feynman_Func_UniformE_V01_SM(laser_width_range(m), laser_res_range(i), laser_pulse_range(l));
            
            assign_var_name = ['Simul_Feynman_Func_V01_',num2str(i),'_',num2str(l),'_',num2str(m)];
            save(assign_var_name,'phase');
            
            [phase] = Simul_Feynman_Dual_Laser_Func_UniformE_V01_SM(laser_width_range(m), laser_res_range(i), laser_pulse_range(l));
