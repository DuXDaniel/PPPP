function [final_phase_data, voxel_grid_phase_data] = Simul_Feynman_Dual_Laser_Func_V01_SM_VarVox(beam_waist, laser_res, e_res, pulse_energy, voxgran, slicegran)
...
min_t_vals = max([1000, slice_granularity]);
if (min_t_vals ~= slice_granularity)
    min_t_vals_sep = round(min_t_vals/slice_granularity) + slice_granularity;
    t_range_extended = zeros(1,slice_granularity*min_t_vals_sep - min_t_vals_sep);
    for l = 1:slice_granularity-1
        t_range_extended((l-1)*min_t_vals_sep+1:l*min_t_vals_sep+1) = linspace(t_range(l),t_range(l+1),min_t_vals_sep+1);
    end
else
...
norm_func_vals = norm_func(t_range_extended);

parfor cur_voxel = 1:num_voxels
    y_vals = init_y_vals(cur_voxel) - vel.*t_range_extended;
    x_vals = y_vals.*x_slopes(cur_voxel);
    z_vals = y_vals.*z_slopes(cur_voxel);
    rho_vals = sqrt(x_vals.^2+y_vals.^2);
    rho_vals_2 = sqrt(z_vals.^2+y_vals.^2);
    density_vals = norm_func_vals.*laser(rho_vals,z_vals,t_range_extended) + norm_func_vals.*laser(rho_vals_2,x_vals,t_range_extended);
    full_vals = hbar.*alpha.*n_pulse.*density_vals.*lambda./sqrt(mass_e^2.*(1+vel.^2/c.^2));
    voxel_grid_phase_data_unpacked(cur_voxel) = trapz(t_range_extended,full_vals);
end
